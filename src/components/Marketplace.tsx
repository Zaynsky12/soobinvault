...
export function Marketplace() {
  const { account, signAndSubmitTransaction, wallet, signMessage } = useWallet();
                    ...
                    blobs(
                        where: { 
                            _or: [
                                ...
                                { blob_name: { _ilike: "%paylink--%" } },
                                { blobName: { _ilike: "%paylink--%" } },
                                { name: { _ilike: "%paylink--%" } }
                            ],
              ...
              if (liveBlobs && liveBlobs.length > 0) {
                const userMarketBlobs = liveBlobs.filter((b: any) => {
                  const n =
                    ...
                    b.name ||
                    "";
                  return n.includes("sv_market--") || n.includes("paylink--");
                });
