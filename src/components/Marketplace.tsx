...
export function Marketplace() {
                        ...
                        if (result?.data?.blobs) {
                            ...
                            if (foundBlobs.length > 0) {
                                ...
                                for (const fb of foundBlobs) {
                                    ...
                                    if (!blobs.some(b => (b.blob_name === bName || b.blobName === bName))) {
                                        blobs.push({
                                            ...fb,
                                            blob_name: bName
                                        });
                                        ...
                                        const existing = blobs.find(b => (b.blob_name === bName || b.blobName === bName));
                    ...
                    try {
                        ...
                        if (liveBlobs && liveBlobs.length > 0) {
                            ...
                            for (const umb of userMarketBlobs) {
                                ...
                                if (!blobs.some(b => (b.blob_name === rawName || b.blobName === rawName))) {
                                    console.log(`[Marketplace] DISCOVERED: ${rawName.slice(0, 20)}... from ${seller.slice(0, 8)}`);
                                    blobs.push({
                                        blob_name: rawName,
                                        owner: seller,
                         ...
                         console.warn(`[Marketplace] Crawl failed for ${seller.slice(0, 8)}:`, e);
                    }
                }
            }
            console.groupEnd();
                ...
                const rawMapped = blobs
                    ...
                    .map((d: any) => {
                        ...
                        return {
                            ...
                            price: price,
                            size: d.size && d.size !== "0" ? `${(parseInt(d.size) / 1024).toFixed(1)} KB` : "Stored Asset",
    ...
    */
    ...
    const handlePurchase = async (dataset: any) => {
        ...
        const actionToastId = toast.loading(dataset.isFree ? `Fetching ${dataset.title}...` : `Initializing P2P purchase for ${dataset.title}...`);
        ...
        try {
            if (dataset.price > 0) {
                ...
                toast.loading(`SUSD payment sent to ${dataset.seller}! Now downloading...`, { id: actionToastId });
            }
